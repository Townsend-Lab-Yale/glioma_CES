...
lolli <- lolli + theme_bw() +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 0, r = 1.5, b = 0, l = 0),size=theme_size),
        axis.text.x = element_text(size=theme_size),
        axis.text.y = element_text(size=theme_size),
        axis.title.y=element_text(size=theme_size)) +
...
# ggsave(filename = "figures/fig_1_glioma_CES.tiff",plot = lolli,width = 15,height = 10)
ggsave(filename = "figures/fig_1_glioma_CES.eps",plot = lolli,width = 15,height = 10,device = cairo_ps)
ggsave(filename = "figures/fig_1_glioma_CES.pdf",plot = lolli,width = 15,height = 10)
...
all_nrsi <- cowplot::plot_grid(plotlist = plot_list,nrow = 2,align = "hv",labels = "AUTO")

# cowplot::save_plot(plot = all_nrsi,filename = "figures/fig_4_summed_nrsi.png",base_height = 6,base_width = 10)

cowplot::save_plot(plot = all_nrsi,filename = "figures/fig_4_summed_nrsi.eps",base_height = 6,base_width = 10,device = cairo_ps)

cowplot::save_plot(plot = all_nrsi,filename = "figures/fig_4_summed_nrsi.pdf",base_height = 6,base_width = 10)
...
save.image("output_data/prior_to_fig2_work.RData")
...
```{r effect function plotter}
# effect function plotter -----


effect_plotter_function <- function(gene_name, 
                                    strip_text = F, 
                                    x_axis_text = F,
                                    log10_TF=T, 
                                    plot_y_axis_breaks=NULL, 
                                    plot_y_axis_labels=NULL,
                                    plot_y_limits = NULL){
  
  CIC <- selection_data_main_clean_wNA_wide_join %>%
    filter(Gene %in% c(gene_name)) 
  
  CIC_long <- selection_data_main_clean_wNA %>%
    filter(Gene %in% gene_name)
  
  
  if(length(grep(pattern = ":",x = CIC$variant_name))>0){
    
    CIC$variant_name[grep(pattern = ":",x = CIC$variant_name)] <- "Noncoding"
  ...
  if(log10_TF){
    CIC <- CIC %>%
      mutate(selection_intensity = log10(selection_intensity))
  ...
  if(is.null(plot_y_limits)){
    plot_y_limits <- c(min(CIC$selection_intensity),max(CIC$selection_intensity))
  ...
  CIC_plot <- ggplot(CIC,
                     aes(x=as.numeric(m_f),y=selection_intensity)) +
    facet_grid(vars(Gene),vars(idh_status),scales = "free",drop=F) +
    geom_text_repel(data = subset(CIC,m_f=="Female"),size=geom.text.size,
                    hjust=1,nudge_x = -0.275,aes(label=variant_name,color=m_f_or_both,x=as.numeric(m_f)),
                    segment.color="gray50",direction = "y",segment.alpha = 0.8,segment.size = 0.2) +
    geom_text_repel(data = subset(CIC,m_f=="Male"),size=geom.text.size,
                    nudge_x = 0.275,aes(label=variant_name,color=m_f_or_both,x=as.numeric(m_f)),
                    segment.color="gray50",direction="y",segment.alpha = .8,hjust=0,segment.size = 0.2) +
    geom_point(aes(color=m_f_or_both,x=as.numeric(m_f)),alpha=0.8,size=4) +
    geom_segment(data=CIC_long,
                 aes(x=1,xend=2,y=Female,yend=Male),color="black",alpha=0.9) +
    ...
    scale_color_manual(values = color_vec_mf) + 
    scale_x_continuous(limits=c(0.25,2.75),breaks = c(1,2),labels = c("Female","Male")) + 
  ...
  if(!is.null(plot_y_axis_breaks)){
    CIC_plot <- CIC_plot + 
      scale_y_continuous(breaks=plot_y_axis_breaks,
                         labels = plot_y_axis_labels,
                         limits = c(plot_y_limits[1],plot_y_limits[2]))
...
```{r figure 2 each gene}

# load("output_data/prior_to_fig2_work.RData")
library(ggrepel);library(scales)
geom.text.size = 3.9
theme.size = 3.2 * geom.text.size+10
annotation_text_size = 5.5

points_per_gene <- selection_data_main_clean_wNA_wide %>%
  count(Gene) %>% 
  arrange(desc(n))

color_vec_mf <- setNames(object = c("#FA8276","#71A5FF","#1CC747") ,nm = c("Female only","Male only","Both sexes"))

# TP53 H179R

selection_data_main_clean_wNA <- selection_data_main_clean_wNA %>%
  mutate(m_f_or_both = case_when(!is.na(Female) & is.na(Male) ~ "Female only",
                                 !is.na(Male) & is.na(Female) ~ "Male only", 
                                 !is.na(Female) & !is.na(Male) ~ "Both sexes"))


selection_data_main_clean_wNA_wide$m_f <- factor(selection_data_main_clean_wNA_wide$m_f, levels = c("Female","Male"))
selection_data_main_clean_wNA_wide_join <- left_join(x = selection_data_main_clean_wNA_wide,
                                                     y = selection_data_main_clean_wNA %>% dplyr::select(idh_status,Name,m_f_or_both),by = c("Name"="Name","idh_status"="idh_status"))



# selection_data_main_clean_wNA %>% filter(Name == "TP53 H179R")
# selection_data_main_clean_wNA_wide %>% filter(Name == "TP53 H179R")
# selection_data_main_clean_wNA_wide_join %>% filter(Name == "TP53 H179R")


# TP53 ---- 

# TP53 <- selection_data_main_clean_wNA_wide_join %>%
#   filter(Gene %in% c("TP53")) #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# TP53_long <- selection_data_main_clean_wNA %>%
#   filter(Gene %in% "TP53") #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# 
# 
# TP53_plot <- ggplot(TP53, 
#                     aes(x=m_f,y=selection_intensity)) + 
#   facet_grid(vars(Gene),vars(idh_status),scales = "free") + 
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both)) +
#   # geom_text(data = subset(selection_data_for_2_1,m_f=="Female"),size=1,
#   #           hjust=1,check_overlap = F,nudge_x = -0.05,aes(label=variant_name)) +
#   # geom_text(data = subset(selection_data_for_2_1,m_f=="Male"),size=1,
#   #           hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
#   
#   geom_text_repel(data = subset(TP53,m_f=="Female"),size=geom.text.size,
#                   hjust=1,nudge_x = -0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction = "y",segment.alpha = 0.8,segment.size = 0.2) +
#   geom_text_repel(data = subset(TP53,m_f=="Male"),size=geom.text.size,
#                   nudge_x = 0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction="y",segment.alpha = .8,hjust=0,segment.size = 0.2) +
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both),alpha=0.8) +
#   geom_point(aes(color=m_f_or_both),alpha=0.8,size=4) +
#   geom_segment(data=TP53_long,
#                aes(x=1,xend=2,y=Female,yend=Male),color="black",alpha=0.9) + 
...
TP53_plot <- effect_plotter_function("TP53",strip_text = T,
                                     plot_y_axis_breaks = c(4,5,6),
                                     plot_y_axis_labels = c("4.0","5.0","6.0"))

plot_text_annot <- TP53_plot$data[which.max(TP53_plot$data$selection_intensity),]
plot_text_annot$idh_status <- "IDH mutant"

TP53_plot <- TP53_plot + 
  geom_text(data = plot_text_annot,aes(x=.255,label="TP53",fontface="bold"),
            size=annotation_text_size,hjust=0) +
...
# EGFR <- selection_data_main_clean_wNA_wide_join %>%
#   filter(Gene %in% c("EGFR")) #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# EGFR_long <- selection_data_main_clean_wNA %>%
#   filter(Gene %in% "EGFR") #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))

#
# EGFR <- EGFR %>%
#   mutate(variant_name = case_when(variant_name == "7:55260458 G>A" ~ "Noncoding",
#                                   TRUE ~ variant_name)) 
# 
# EGFR_plot <- ggplot(EGFR, 
#                     aes(x=m_f,y=selection_intensity)) + 
#   facet_grid(vars(Gene),vars(idh_status),scales = "free",drop=F) + 
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both)) +
#   # geom_text(data = subset(selection_data_for_2_1,m_f=="Female"),size=1,
...
#   #           hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
#   
#   geom_text_repel(data = subset(EGFR,m_f=="Female"),size=geom.text.size,
#                   hjust=1,nudge_x = -0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction = "y",segment.alpha = 0.8,segment.size = 0.2) +
#   geom_text_repel(data = subset(EGFR,m_f=="Male"),size=geom.text.size,
#                   nudge_x = 0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction="y",segment.alpha = .8,hjust=0,segment.size = 0.2) +
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both),alpha=0.8) +
#   geom_point(aes(color=m_f_or_both),alpha=0.8,size=4) +
#   geom_segment(data=EGFR_long,
#                aes(x=1,xend=2,y=Female,yend=Male),color="black",alpha=0.9) + 
#   scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x))) +
#   labs(y="Cancer effect size", x="Sex") +
#   theme_bw() + 
...
EGFR_plot <- effect_plotter_function("EGFR",x_axis_text = T,
                                     plot_y_axis_breaks = c(3.5,4.5),plot_y_axis_labels = c("3.5","4.5"))


plot_text_annot <- EGFR_plot$data[which.max(EGFR_plot$data$selection_intensity),]
plot_text_annot$idh_status <- "IDH mutant"

EGFR_plot <- EGFR_plot + 
  geom_text(data = plot_text_annot,aes(x=.255,label="EGFR",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())


ab <- cowplot::plot_grid(TP53_plot,EGFR_plot,nrow = 2,labels = c("A","B"))



# PTEN ---- 

# PTEN <- selection_data_main_clean_wNA_wide_join %>%
#   filter(Gene %in% c("PTEN")) #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# PTEN_long <- selection_data_main_clean_wNA %>%
#   filter(Gene %in% "PTEN") #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# 
# 
...
#   #           hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
# 
#   geom_text_repel(data = subset(PTEN,m_f=="Female"),size=geom.text.size,
#                   hjust=1,nudge_x = -0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction = "y",segment.alpha = 0.8,segment.size = 0.2) +
#   geom_text_repel(data = subset(PTEN,m_f=="Male"),size=geom.text.size,
#                   nudge_x = 0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction="y",segment.alpha = .8,hjust=0,segment.size = 0.2) +
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both),alpha=0.8) +
#   geom_point(aes(color=m_f_or_both),alpha=0.8,size=4) +
#   geom_segment(data=PTEN_long,
#                aes(x=1,xend=2,y=Female,yend=Male),color="black",alpha=0.9) +
#   scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x))) +
#   labs(y="Cancer effect size", x="Sex") +
#   theme_bw() +
...
selection_data_main_clean_wNA_wide %>% filter(Gene == "PTEN") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot) %>% summary()
PTEN_plot <- effect_plotter_function("PTEN",strip_text = T,
                                     plot_y_axis_breaks = c(2.5,3.5,4.5), plot_y_axis_labels = c(2.5,3.5,4.5),plot_y_limits = c(2.8,4.5))
                                     # plot_y_limits = c(2,5))


plot_text_annot <- PTEN_plot$data[which.max(PTEN_plot$data$selection_intensity),]
plot_text_annot$idh_status <- "IDH mutant"

PTEN_plot <- PTEN_plot + 
  geom_text(data = plot_text_annot,aes(x=.255,y=4.4,label="PTEN",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
# PIK3CA <- selection_data_main_clean_wNA_wide_join %>%
#   filter(Gene %in% c("PIK3CA")) #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# PIK3CA_long <- selection_data_main_clean_wNA %>%
#   filter(Gene %in% "PIK3CA") #%>%
# # mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
# 
# 
# # EGFR <- EGFR %>%
...
#   #           hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
#   
#   geom_text_repel(data = subset(PIK3CA,m_f=="Female"),size=geom.text.size,
#                   hjust=1,nudge_x = -0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction = "y",segment.alpha = 0.8,segment.size = 0.2) +
#   geom_text_repel(data = subset(PIK3CA,m_f=="Male"),size=geom.text.size,
#                   nudge_x = 0.25,aes(label=variant_name,color=m_f_or_both),
#                   segment.color="gray50",direction="y",segment.alpha = .8,hjust=0,segment.size = 0.2) +
#   # geom_point(aes(shape=`Variant type`,color=m_f_or_both),alpha=0.8) +
#   geom_point(aes(color=m_f_or_both),alpha=0.8,size=4) +
#   geom_segment(data=PIK3CA_long,
#                aes(x=1,xend=2,y=Female,yend=Male),color="black",alpha=0.9) +
#   scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x))) +
#   labs(y="Cancer effect size", x="Sex") +
#   theme_bw() +
...
PIK3CA_plot <- effect_plotter_function("PIK3CA",
                                       plot_y_axis_breaks = c(4,4.5),plot_y_axis_labels = c("4.0","4.5"),plot_y_limits = c(3.5,5))


plot_text_annot <- PIK3CA_plot$data[which.max(PIK3CA_plot$data$selection_intensity),]
plot_text_annot$idh_status <- "IDH mutant"

PIK3CA_plot <- PIK3CA_plot + 
  geom_text(data = plot_text_annot,aes(x=.255,y=4.9,label="PIK3CA",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
CIC_plot <- effect_plotter_function("CIC", 
                                    plot_y_axis_breaks = c(4,5),
                                    plot_y_axis_labels = c("4.0","5.0"),
                                    plot_y_limits = c(3.7,6))

plot_text_annot <- CIC_plot$data[which.max(CIC_plot$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

CIC_plot <- CIC_plot + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=5.78,label="CIC",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
IDH1 <- effect_plotter_function("IDH1",plot_y_axis_breaks = c(6,7),
                                plot_y_axis_labels = c("6.0","7.0"))


plot_text_annot <- IDH1$data[which.max(IDH1$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

IDH1 <- IDH1 + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=7.4,label="IDH1",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())

# ATRX ---- 

ATRX <- effect_plotter_function("ATRX",plot_y_axis_breaks = c(3,4),plot_y_axis_labels = c("3.0","4.0"),x_axis_text = T)


plot_text_annot <- ATRX$data[which.max(ATRX$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

ATRX <- ATRX + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=4.15,label="ATRX",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())


cdefg <- cowplot::plot_grid(PTEN_plot,PIK3CA_plot,CIC_plot,IDH1,ATRX,ncol = 1,labels = c("C","D","E","F","G"))




abcdefg <- cowplot::plot_grid(ab,cdefg,nrow = 1)


cowplot::save_plot(plot = abcdefg,filename = "output_data/abcdefg.png",base_height = 10,base_width = 10)

# PIK3R1 -----
PIK3R1 <- effect_plotter_function("PIK3R1",x_axis_text = F,
                                  plot_y_axis_breaks = c(4,4.5),plot_y_axis_labels = c("4.0","4.5"),strip_text = T)

plot_text_annot <- PIK3R1$data[which.max(PIK3R1$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

PIK3R1 <- PIK3R1 + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=4.5,label="PIK3R1",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
# IDH2 ----
IDH2 <- effect_plotter_function("IDH2",strip_text = F,
                                plot_y_axis_breaks = c(4.5,5),plot_y_axis_labels = c("4.5","5.0"),
                                plot_y_limits = c(4.2,5.2))

plot_text_annot <- IDH2$data[which.max(IDH2$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

IDH2 <- IDH2 + 
  geom_text(data = plot_text_annot[1,],aes(y=5.15,x=.255,label="IDH2",fontface="bold"), size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
# NF1 -----
NF1 <- effect_plotter_function("NF1",plot_y_axis_breaks = c(3.0,3.4),plot_y_axis_labels = c("3.0","3.4"),plot_y_limits = c(2.5,3.75))


plot_text_annot <- NF1$data[which.max(NF1$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

NF1 <- NF1 + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,label="NF1",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())

# PTPN11 -----
PTPN11 <- effect_plotter_function("PTPN11", plot_y_axis_breaks = c(3.4,3.8),
                                  plot_y_axis_labels = c("3.4","3.8"),plot_y_limits = c(3.3,4.1))

plot_text_annot <- PTPN11$data[which.max(PTPN11$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

PTPN11 <- PTPN11 + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=4.05,label="PTPN11",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())
...
# TRAT1 ----
TRAT1 <- effect_plotter_function("TRAT1",plot_y_axis_breaks = c(3,4),plot_y_axis_labels = c("3.0","4.0"),plot_y_limits = c(2.9,4.75),x_axis_text = T)

plot_text_annot <- TRAT1$data[which.max(TRAT1$data$selection_intensity),]
plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
plot_text_annot$idh_status[1] <- "IDH mutant"
plot_text_annot$idh_status[2] <- "IDH wildtype"

TRAT1 <- TRAT1 + 
  geom_text(data = plot_text_annot[1,],aes(x=.255,y=4.6,label="TRAT1",fontface="bold"),
            size=annotation_text_size,hjust=0) +
  theme(strip.text.y = element_blank())



hijkl <- cowplot::plot_grid(PIK3R1,IDH2,NF1,PTPN11,TRAT1,ncol = 1,labels = c("H","I","J","K","L"))

al <- cowplot::plot_grid(ab,cdefg,hijkl,nrow = 1)

cowplot::save_plot(plot=al,filename = "output_data/al.png",base_height = 10,base_width = 15)
# 
# # H3F3A ----
# 
# selection_data_main_clean_wNA_wide %>% filter(Gene == "H3F3A") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# 
# H3F3A <- effect_plotter_function("H3F3A",plot_y_axis_breaks = c(3.8,3.9),
#                                  plot_y_axis_labels = c("3.8","3.9"),plot_y_limits = c(3.79,3.94))
# 
# plot_text_annot <- H3F3A$data[which.max(H3F3A$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# H3F3A <- H3F3A + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=3.914,label="H3F3A",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# 
# 
# # KIF2B -----
# selection_data_main_clean_wNA_wide %>% filter(Gene == "KIF2B") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# KIF2B <- effect_plotter_function("KIF2B", plot_y_axis_breaks = c(3.1,3.2), plot_y_axis_labels = c("3.1","3.2"),plot_y_limits = c(3.09,3.24))
# 
# plot_text_annot <- KIF2B$data[which.max(KIF2B$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# KIF2B <- KIF2B + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=3.21,label="KIF2B",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# # PDGFRA ----
# 
# selection_data_main_clean_wNA_wide %>% filter(Gene == "PDGFRA") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# 
# PDGFRA <- effect_plotter_function("PDGFRA",plot_y_axis_breaks = c(3.2,3.3),plot_y_axis_labels = c("3.2","3.3"),plot_y_limits = c(3.19,3.34))
# 
# plot_text_annot <- PDGFRA$data[which.max(PDGFRA$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# PDGFRA <- PDGFRA + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=3.31,label="PDGFRA",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# # RB1 ----
# 
# selection_data_main_clean_wNA_wide %>% filter(Gene == "RB1") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# 
# RB1 <- effect_plotter_function("RB1",plot_y_axis_breaks = c(3.3,3.4),plot_y_axis_labels = c("3.3","3.4"),plot_y_limits = c(3.29,3.44))
# 
# plot_text_annot <- RB1$data[which.max(RB1$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# RB1 <- RB1 + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=3.41,label="RB1",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# # SPATA31D1 ----
# selection_data_main_clean_wNA_wide %>% filter(Gene == "SPATA31D1") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# SPATA31D1 <- effect_plotter_function("SPATA31D1",plot_y_axis_breaks = c(4.0,4.1),plot_y_axis_labels = c("4.0","4.1"),plot_y_limits = c(3.99,4.14))
# 
# plot_text_annot <- SPATA31D1$data[which.max(SPATA31D1$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# SPATA31D1 <- SPATA31D1 + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=4.11,label="SPATA31D1",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# # STAG2 -----
# selection_data_main_clean_wNA_wide %>% filter(Gene == "STAG2") %>% mutate(for_plot = log10(selection_intensity)) %>% pull(for_plot)
# STAG2 <- effect_plotter_function("STAG2",x_axis_text = T,plot_y_axis_breaks = c(3.5,3.6),plot_y_axis_labels = c("3.5","3.6"),plot_y_limits = c(3.49,3.64) )
# 
# plot_text_annot <- STAG2$data[which.max(STAG2$data$selection_intensity),]
# plot_text_annot <- rbind(plot_text_annot,plot_text_annot)
# plot_text_annot$idh_status[1] <- "IDH mutant"
# plot_text_annot$idh_status[2] <- "IDH wildtype"
# 
# STAG2 <- STAG2 + 
#   geom_text(data = plot_text_annot[1,],aes(x=.255,y=3.61,label="STAG2",fontface="bold"),
#             size=annotation_text_size,hjust=0) +
#   theme(strip.text.y = element_blank())
# 
# cdefgh <- cowplot::plot_grid(PTEN_plot,PIK3CA_plot,CIC_plot,IDH1,ATRX,PIK3R1,ncol = 1,labels = c("C","D","E","F","G","H"))
# 
# ir <- cowplot::plot_grid(
#   IDH2,NF1,PTPN11,TRAT1,H3F3A,KIF2B,PDGFRA,RB1,SPATA31D1,STAG2,ncol=1,labels=LETTERS[9:18])
# 
# a_r <- cowplot::plot_grid(ab,cdefgh,ir,nrow = 1)


# cowplot::save_plot(plot = a_r,filename = "output_data/Fig_2_a_r.png",base_height = 11,base_width = 20)

# cowplot::save_plot(plot = a_r,filename = "output_data/Fig_2_a_r.eps",base_height = 11,base_width = 20,device=cairo_ps)
# cowplot::save_plot(plot = a_r,filename = "figures/Fig_2_a_r.png",base_height = 11,base_width = 20)
...
y_label <- ggdraw() +
  draw_label(
    "Cancer effect size (log10 scale)",
    fontface = 'bold',
    x = 0,
    angle=90,
    y=0.5,
...
# cowplot::save_plot(plot = a_l_lab,filename = "figures/Fig_2_a_r.png",base_height = 12,base_width = 20)


cowplot::save_plot(plot = a_l_lab,filename = "figures/Fig_2_a_l.eps",base_height = 12,base_width = 20,device=cairo_ps)
cowplot::save_plot(plot = a_l_lab,filename = "figures/Fig_2_a_l.pdf",base_height = 12,base_width = 20)


# a_c <- cowplot::plot_grid(TP53_plot,EGFR_plot,PTEN_plot,ncol = 1,labels = c("A","B","C"))
# 
# c_i <- cowplot::plot_grid(PIK3CA_plot,CIC_plot,IDH1,ATRX,PIK3R1,IDH2,ncol = 1,labels = c("D","E","F","G","H","I"))
# 
# j_r <- cowplot::plot_grid(NF1,PTPN11,TRAT1,H3F3A,KIF2B,PDGFRA,RB1,SPATA31D1,STAG2,ncol=1,labels=LETTERS[10:18])
# 
# a_r <- cowplot::plot_grid(a_c,c_i,j_r,nrow = 1)
# 
# cowplot::save_plot(plot = a_r,filename = "output_data/a_r2.png",base_height = 11,base_width = 20)
...
sig_w_for_stats <- signature_weights %>%
...
wilcox.test(SBS42 ~ m_f, data= sig_w_for_stats,alternative="two.sided")


ggplot(sig_w_for_stats, aes(x=sig_extraction_snvs, y=SBS42,fill=m_f)) +
  # geom_violin() +
  geom_jitter(shape=21,color="black",alpha=0.5,height = .01) +
  geom_vline(xintercept = 50,color="red") + 
  theme_classic() +
  scale_x_log10() +
  labs(x="SNVs in the tumor",y="Occupational exposure to haloalkanes (42) \nsignature weight")

# 
# halo_sigs_plot_stats <- ggplot(sig_w_for_stats, aes(x=m_f, y=SBS42)) +
#   geom_violin() +
...
halo_sigs_w_zero_count <- halo_sigs_w_zero %>%
  group_by(tumor_type) %>%
  mutate(zero_or_no = case_when(SBS42 == 0 ~ 0.0, 
                                SBS42 > 0 ~ 0.025)) %>%
  count(zero_or_no) %>%
  mutate(n = case_when(zero_or_no > 0 ~ paste0("↑",as.character(n)),
                       TRUE ~ paste0("←",as.character(n))))



halo_sigs_plot_w_zero_jitter <- ggplot(halo_sigs_w_zero, aes(x=tumor_type, y=SBS42)) + 
  geom_jitter(aes(color=SBS42),height = 0,width = 0.2,alpha=0.8) + 
  geom_text(data = halo_sigs_w_zero_count,aes(x=as.numeric(tumor_type)+0.6,y=zero_or_no,label=n),hjust=1) +
  theme_classic() + 
  labs(x="Tumor type",y="Occupational exposure to haloalkanes \n(COSMIC 42) signature weight") + 
  scale_color_gradient(low="gray10",high = "darkred") + 
  theme(legend.position = "none")


ggsave(filename = "figures/fig_5_halo_sigs_jitter.png",plot = halo_sigs_plot_w_zero_jitter,width = 6,height = 4)
...
selection_data_main_for_fig3$tumor_type <- forcats::fct_recode(selection_data_main_for_fig3$tumor_type,
                                                   `IDH mutant, Female` = "IDH_mutant_F",
                                                   `IDH mutant, Male` = "IDH_mutant_M",
                                                   `IDH wildtype, Female` = "IDH_WT_F",
                                                   `IDH wildtype, Male` = "IDH_WT_M"
...
selection_data_main_for_fig3$tumor_type

ggplot(selection_data_main_for_fig3,aes(x=selection_intensity,y=fct_rev(tumor_type))) + 
  geom_jitter(aes(shape=`Variant type`,color=`Variant type`),
              alpha=0.9,width = 0,height = .3) + 
  scale_x_log10(breaks=c(1e3,1e5,1e7),labels=expression(10^3,10^5,10^7)) + 
  theme_bw() + 
  scale_fill_discrete(name = "Variant type") + 
  labs(y="Tumor type", x="Cancer effect size") -> 
  selection_v_variant_type

ggsave(filename = "figures/fig_3_selection_v_variant_type.pdf",
       plot = selection_v_variant_type,height = 3,width = 8)

ggsave(filename = "figures/fig_3_selection_v_variant_type.eps",
       plot = selection_v_variant_type,height = 3,width = 8,device = cairo_ps)
