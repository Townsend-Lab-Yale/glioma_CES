...
# thanks to https://stackoverflow.com/a/25062509 
geom.text.size = 3
...
selection_data_for_2_1 <- selection_data_main_clean_wNA_wide %>%
  filter(Gene %in% points_per_gene$Gene[1:2]) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[1:2]))
selection_data_long_2_1 <- selection_data_main_clean_wNA %>%
  filter(Gene %in% selection_data_for_2_1$Gene) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[1:2]))


# leftmost, so no legend... -----
effect_size_by_sex_by_gene_1 <- ggplot(selection_data_for_2_1,
                                       aes(x=m_f,y=selection_intensity)) + 
  facet_grid(vars(Gene),vars(idh_status),scales = "free") + 
  geom_point(aes(shape=`Variant type`,color=`Variant type`)) +
  ...
  geom_text_repel(data = subset(selection_data_for_2_1,m_f=="Female"),size=geom.text.size,
                  hjust=1,check_overlap = F,nudge_x = -0.05,aes(label=variant_name)) +
  geom_text_repel(data = subset(selection_data_for_2_1,m_f=="Male"),size=geom.text.size,
                  hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
  geom_segment(data=selection_data_long_2_1,
               aes(x=1,xend=2,y=Female,yend=Male),color="gray50",alpha=0.5) + 
  scale_y_log10() + 
  labs(y="Cancer effect size", x="Sex") +
  theme_bw() + 
  theme(strip.text.y = element_text(angle = 90))

ggsave(plot = effect_size_by_sex_by_gene_1,filename = "figures/test_fig2.png")

fig_2_legend <- cowplot::get_legend(effect_size_by_sex_by_gene_1)

effect_size_by_sex_by_gene_1 <- effect_size_by_sex_by_gene_1 + 
...
# middle, so x axis title ---- 




selection_data_for_2_2 <- selection_data_main_clean_wNA_wide %>%
  filter(Gene %in% points_per_gene$Gene[3:6]) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[3:6]))
selection_data_long_2_2 <- selection_data_main_clean_wNA %>%
  filter(Gene %in% selection_data_for_2_2$Gene) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[3:6]))


# leftmost, so no legend... -----
effect_size_by_sex_by_gene_2 <- ggplot(selection_data_for_2_2,
                                       aes(x=m_f,y=selection_intensity)) + 
  facet_grid(vars(Gene),vars(idh_status),scales = "free") + 
  geom_point(aes(shape=`Variant type`,color=`Variant type`)) +
  # geom_text(data = subset(selection_data_for_2_1,m_f=="Female"),size=1,
  ...
  geom_text_repel(data = subset(selection_data_for_2_2,m_f=="Female"),size=geom.text.size,
                  hjust=1,check_overlap = F,nudge_x = -0.05,aes(label=variant_name)) +
  geom_text_repel(data = subset(selection_data_for_2_2,m_f=="Male"),size=geom.text.size,
                  hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
  geom_segment(data=selection_data_long_2_2,
               aes(x=1,xend=2,y=Female,yend=Male),color="gray50",alpha=0.5) + 
  scale_y_log10() + 
  labs(y="Cancer effect size", x="Sex") +
  theme_bw() + 
...
selection_data_for_2_3 <- selection_data_main_clean_wNA_wide %>%
  filter(Gene %in% points_per_gene$Gene[7:nrow(points_per_gene)]) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))
selection_data_long_2_3 <- selection_data_main_clean_wNA %>%
  filter(Gene %in% selection_data_for_2_3$Gene) %>%
  mutate(Gene = forcats::fct_relevel(Gene,points_per_gene$Gene[7:nrow(points_per_gene)]))


# leftmost, so no legend... -----
effect_size_by_sex_by_gene_3 <- ggplot(selection_data_for_2_3,
                                       aes(x=m_f,y=selection_intensity)) + 
  facet_grid(vars(Gene),vars(idh_status),scales = "free") + 
  geom_point(aes(shape=`Variant type`,color=`Variant type`)) +
  # geom_text(data = subset(selection_data_for_2_1,m_f=="Female"),size=1,
  ...
  geom_text_repel(data = subset(selection_data_for_2_3,m_f=="Female"),size=geom.text.size,
                  hjust=1,check_overlap = F,nudge_x = -0.05,aes(label=variant_name)) +
  geom_text_repel(data = subset(selection_data_for_2_3,m_f=="Male"),size=geom.text.size,
                  hjust=0,check_overlap = F,nudge_x = 0.05,aes(label=variant_name)) +
  geom_segment(data=selection_data_long_2_3,
               aes(x=1,xend=2,y=Female,yend=Male),color="gray50",alpha=0.5) + 
  scale_y_log10() + 
  labs(y="Cancer effect size", x="Sex") +
  theme_bw() + 
...
effect_size_by_sex_by_gene_all <- cowplot::plot_grid(effect_size_by_sex_by_gene_1,
                                                     effect_size_by_sex_by_gene_2,
                                                     effect_size_by_sex_by_gene_3,
                                                     fig_2_legend,rel_widths = c(1,1,1,.3),nrow = 1)

cowplot::save_plot(plot = effect_size_by_sex_by_gene_all,
                   filename = "figures/test_fig2.png",base_height = 8,base_width = 15)
...
effect_plotter_function <- function(gene_name, strip_text = F, x_axis_text = F,log10_TF=T, plot_y_axis_breaks=NULL, plot_y_axis_labels=NULL,plot_y_limits = NULL){
...
TP53_plot <- effect_plotter_function("TP53",strip_text = T,
                                     plot_y_axis_breaks = c(4,5,6),plot_y_axis_labels = c("4.0","5.0","6.0"))
# EGFR ----- 
